...
db.settings({timestampsInSnapshots : true});
  ...
  function call()
       {
           var locationnews = document.getElementById("locationnews").value;

              db.collection("posts").where("newsLocationEnglish", "==", locationnews).where("approvedEnglish", "==", true).where("approvedHindi", "==", true).orderBy("createdAt", "desc").limit(39).get().then(function (querySnapshot) {
              var i = 0;
              var getOptions = {
    source: 'cache'
};
                  ...
                  var lastVisible = querySnapshot.docs[querySnapshot.docs.length-1];
                      window.addEventListener('scroll', ()=>{
                          ...
                          if(Math.ceil(scrolled) === scrollable){
                              ...
                              db.collection("posts").where("newsLocationEnglish", "==", locationnews).where("approvedEnglish", "==", true).where("approvedHindi", "==", true).orderBy("createdAt", "desc").startAfter(lastVisible).limit(3).onSnapshot(function (snapshots) {
