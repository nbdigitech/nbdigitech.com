 ...
 */
if (!function_exists('get_trash_duplicates_total_downloads')) {

    function get_trash_duplicates_total_downloads() {
...
if ($wp_version > 3.8) {
    if (!function_exists('wp_trash_duplicates_star_rating')) {

        function wp_trash_duplicates_star_rating($args = array()) {
            $plugins = $response = "";
            $args = array(
                'author' => 'solwininfotech',
                'fields' => array(
...
if (!function_exists('TDRD_admin_func')) {
    ...
    function TDRD_admin_func() {
        ...
        $trash_count_record = "SELECT COUNT(first.post_title) FROM ( SELECT post_title, post_type FROM $tbl_nm WHERE $trash_post_type $main_search_query AND $show_drafts_query GROUP BY post_title,post_type HAVING COUNT(*)>1 ) AS first INNER JOIN ( SELECT post_title, post_type FROM $tbl_nm WHERE $trash_post_type $main_search_query AND $show_drafts_query GROUP BY post_title,post_type HAVING COUNT(*)>1 )AS second ON first.post_title = second.post_title AND first.post_type = second.post_type";
        ...
        $trash_duplicates_query = "SELECT * FROM $tbl_nm AS first INNER JOIN ( SELECT post_title,post_type FROM $tbl_nm WHERE $trash_post_type $main_search_query AND $show_drafts_query GROUP BY post_title,post_type HAVING COUNT(*)>1 LIMIT $offset, $show_num ) AS second ON first.post_title = second.post_title AND first.post_type = second.post_type
            WHERE " . str_replace($tbl_nm, 'first', $trash_post_type) . " AND " . str_replace($tbl_nm, 'first', $show_drafts_query) . $main_search_query2 . "
            ORDER BY first.post_title, first.post_type DESC";
        $trash_final_result = $wpdb->get_results($trash_duplicates_query, ARRAY_A);

        $count_individual_query = "SELECT COUNT(*) FROM $tbl_nm AS first INNER JOIN ( SELECT post_title,post_type FROM $tbl_nm WHERE $trash_post_type $main_search_query AND $show_drafts_query GROUP BY post_title,post_type HAVING COUNT(*)>1 ) AS second ON first.post_title = second.post_title AND first.post_type = second.post_type
            WHERE " . str_replace($tbl_nm, 'first', $trash_post_type) . " AND " . str_replace($tbl_nm, 'first', $show_drafts_query) . $main_search_query2 . "
            ORDER BY first.post_title, first.post_type DESC";
                        ...
                        <table cellspacing="0" class="wp-list-table widefat fixed striped">
                            ...
                            <tbody class="trash_group">
                                ...
                                foreach ($trash_final_result as $key => $value) :
                                    if ($key != 0) {
                                        $key_z = trim(strtolower($trash_final_result[$key - 1]['post_title'])) . '_' . trim(strtolower($trash_final_result[$key - 1]['post_type']));
                                    }
                                    if ($key_z != trim(strtolower($value['post_title'])) . '_' . trim(strtolower($value['post_type']))) {
                                        ?>
                    ...
                    <?php if ($pages_all > 0) { ?>
                        <div class="trash-inner-bottom">
                            <div class="tablenav">
                                ...
                                <div class="tablenav-pages float_right">
                                    <?php $list = isset($_GET['number']) ? "&number=" . $_GET['number'] : ""; ?>
                                    <span class="pagination-links">
                                        <?php if ($page == '1') { ?>
                                            <span class="tablenav-pages-navspan" aria-hidden="true">&laquo;</span>
                                            <span class="tablenav-pages-navspan" aria-hidden="true">&lsaquo;</span>
...
if (!function_exists('TDRD_selected')) {
    ...
    function TDRD_selected($trash_sel_id) {
        ...
        if ($trash_sel_id) :
            $remove_id = $_REQUEST['chk_remove_sel'];
...
if (!function_exists('TDRD_trash_all')) {
    ...
    function TDRD_trash_all() {
        ...
        if (isset($_REQUEST['duplicate-items-apply']) && isset($_REQUEST['delete-all-duplicates'])) {
            ...
            $postIds = array();
            $postIds = $_REQUEST['select_posts_hidden'];
...
//                        $newest = array_keys($publishDt, end($publishDt));
                        ...
                        foreach ($oldest as $pid) {
                            ...
                            if ($st == 'draft') {
                                ...
                                $new_uri = get_permalink($newest[0]);
                                $home = home_url();
                ...
                case "trash_newest":
                    foreach ($groupArr as $val) {
                        ...
                        $array_keys = array_keys($publishDt);
                        $oldest = $array_keys[0];
//                        $oldest = array_keys($publishDt, current($publishDt));
                        foreach ($publishDt as $k => $v) {
                            if ($k != $oldest) {
                                $newest[$n] = $k;
                                $n++;
                        ...
                        foreach ($newest as $pid) {
                            ...
                            if ($st == 'draft') {
                                ...
                                $post_type = get_post_type($pid);
                                $post_permalink = get_permalink($pid);
                                $new_uri = get_permalink($oldest[0]);
                                $home = home_url();
                                $old_url = str_replace($home, '', $post_permalink);
                                $ids[] = $pid;
            ...
            foreach ($_POST as $key => $value) {
                if (strspn($key, 'duplicate_apply_') == 16) {
                    ...
                    $keep_type = $_REQUEST['duplicate_keep'];
