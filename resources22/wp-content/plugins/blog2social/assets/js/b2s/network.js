    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            if (data.result == true && data.mandantId >= 1) {
                ...
                jQuery(".b2s-network-mandant-select option[value='0']").prop('selected', true);
...
function loginSuccess(networkId, networkType, displayName, networkAuthId, mandandId) {
    ...
    if (jQuery('.b2s-network-item-auth-list-li[data-network-auth-id="' + networkAuthId + '"').length == 0) {
        ...
        html += displayName + ' <a class="b2s-network-item-auth-list-btn-delete" data-network-auth-id="' + networkAuthId + '" href="#">' + jQuery('#b2sJsTextRemove').val() + '</a></li>';
