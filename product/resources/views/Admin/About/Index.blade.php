            ...
            <div class="col-md-12">
              <div class="card">
                  ...
                  <div style="background-color: #5BBC2E; padding: 10px; padding-left: 16px; padding-top:16px; border-radius: 3px;" style="width: 100%;">
                    <h4>Manage About
                      <a href="{{route('Admin/About/Add')}}" class="btn btn-success pull-right" style="margin-top: -7px; background-color: white; color:#5BBC2E;">Add New</a></h4>
                  </div>
                </div>
                <div class="card-body">
              ...
              @endif
                  ...
                  @foreach($Abouts as $row)
                        <tr>
                          <td>{{$i++}}</td>
                          <td>{{$row->Title}}</td>
                          <td><img src="https://s3.ap-south-1.amazonaws.com/nbpaperplanes/images/{{$row->Image}}" style="width:120px;height:70px;"></td>
                          <td>
                             @if($row->Status == 1)
                                <i class="fa fa-circle" style="color:lime"></i>
                              @else
                          ...
                          <form action="{{route('Admin/About/EditSession')}}" method="post">
                              {{csrf_field()}}
                          <td class="td-actions text-right">
                              <input type="hidden" name="Edit" value="{{$row->id}}">
                            <button type="submit" rel="tooltip" class="btn btn-success" style="background-color: #5BBC2E; padding:10px; padding-left: 20px; margin-right: 10px; padding-right: 20px;">
                              Edit
                            </button>
                            </form>
                            <button type="button" rel="tooltip" data-toggle="modal" data-target="#myModal" class="btn btn-danger pull-right" onclick="delete_function('{{$row->id}}')" style="background-color: red; padding:10px; padding-left: 15px; padding-right: 15px;">
                              Delete
                            </button>
                          </td>
                        </tr>
...
<div class="modal" id="myModal">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <!-- Modal Header -->
      <div class="modal-header"  style="background-color: #5BBC2E; color:white;">
