  ...
  function call()
       {
           ...
           var num = 1;
           var first = db.collection("posts").where("approvedEnglish", "==", true).where("approvedHindi", "==", true).orderBy("createdAt", "desc").limit(39).get().then(function (querySnapshot) {
              var i = 0;
              var getOptions = {
    source: 'cache'
};
                  ...
                  var lastVisible = querySnapshot.docs[querySnapshot.docs.length-1];
                      window.addEventListener('scroll', ()=>{
                          ...
                          if(Math.ceil(scrolled) === scrollable){
                              ...
                              var next = db.collection("posts").where("approvedEnglish", "==", true).where("approvedHindi", "==", true).orderBy("createdAt", "desc").startAfter(lastVisible).limit(3).onSnapshot(function (snapshots) {
