...
<div class="popShAtt" id="popShAtt<?php echo $ii; ?>X"><h3><?php _e('Facebook Post Types', 'social-networks-auto-poster-facebook-twitter-g'); ?></h3><img src="<?php echo NXS_PLURL; ?>img/fbPostTypesDiff6.png" width="600" height="398" alt="<?php _e('Facebook Post Types', 'social-networks-auto-poster-facebook-twitter-g'); ?>"/></div>
  ...
  function adjPreFormatWP(&$options, $postID){  $post = get_post($postID); 
    ...
    if ($options['attachVideo']=='A') {$vids = nsFindVidsInPost($post, false); if (count($vids)>0) {
      if (strlen($vids[0])==11) { $vidURL = 'http://www.youtube.com/v/'.$vids[0];  $options['imgToUse'] = nsGetYTThumb($vids[0]); }
      if (strlen($vids[0])==8 || strlen($vids[0])==9) { // $vidURL = 'https://secure.vimeo.com/moogaloop.swf?clip_id='.$vids[0].'&autoplay=1';            
        ...
        $apiURL = "http://vimeo.com/api/v2/video/".$vids[0].".json?callback=showThumb"; $json = nxs_remote_get($apiURL, $wprg);
        if (!is_nxs_error($json)) { $json = $json['body'];  $json = str_replace('/**/','',$json);
        $json = str_replace('showThumb(','',$json); $json = str_replace('])',']',$json);  $json = json_decode($json, true);  $options['imgToUse'] = $json[0]['thumbnail_large']; }           
      }
      if (strlen($vids[0])==15) { // $vidURL = 'https://secure.vimeo.com/moogaloop.swf?clip_id='.$vids[0].'&autoplay=1';            
        $vidURL = 'https://www.facebook.com/video.php?v='.$vids[0]; $apiURL = "https://graph.facebook.com/v2.3/".$vids[0]; $json = nxs_remote_get($apiURL, $wprg);
        if (!is_nxs_error($json)) { $json = $json['body']; $json = json_decode($json, true); $frmts = $json['format']; $imgURL = array_pop($frmts);  $options['imgToUse'] = $imgURL['picture'];  }           
      }
    }} 
    ...
    if (!empty($postID)) { if (trim($options['imgToUse'])!='') $imgURL = $options['imgToUse']; else $imgURL = nxs_getPostImage($postID, !empty($options['wpImgSize'])?$options['wpImgSize']:'full');
      ...
      if ( ($options['postType']=='A' || $options['postType']=='S') && $options['attachInfo']!='F' ){ $lng = '';
        ...
        if ($options['attachInfo']=='M') {$dsc = nsFormatMessage($options['msgATFormat'], $postID, $addParams); $urlTitle = nsFormatMessage($options['msgAFormat'], $postID, $addParams);  } 
          ...
          if (empty($dsc)) $dsc = trim(nxs_doQTrans($post->post_excerpt, $lng));  if (empty($dsc)) $dsc = trim(nxs_doQTrans($post->post_content, $lng));  
