...
*/
define( 'NextScripts_SNAP_Version' , '3.8.5' ); define( 'NextScripts_SNAP_Version_Date' , 'June 10, 2017' ); 
...
function nxs_ogtgCallback($content){ global $post, $plgn_NS_SNAutoPoster;  
  if (stripos($content, 'og:title')!==false) $ogOut = "\r\n"; else {
    ...
    if (!empty($options['fb']) && count($options['fb'])>0) $k = array_shift(array_slice($options['fb'], 0, 1)); $ogappID = (!empty($k) && !empty($k['appKey']))  ? ('<meta property="fb:app_id" content="'.$k['appKey'].'"/>'."\r\n"):'';
    ...
    $ogOut  = "\r\n".$ogSN.$ogT.$ogD.$ogType.$ogUrl.$ogLoc.$ogImgsOut.$ogappID;
