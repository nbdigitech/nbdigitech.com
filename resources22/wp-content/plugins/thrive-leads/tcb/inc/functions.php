...
function tve_save_post() {
	...
	if ( isset( $_POST['post_id'] ) && current_user_can( 'edit_post', $_POST['post_id'] ) ) {
		...
		/**
		 * put the current content into this meta to be used in search filter
		 *
		 * @see tve_process_search_clauses
		 */
		update_post_meta( $_POST['post_id'], 'tve_search_content', $_POST['tve_content'] );
...
function tve_editor_content( $content, $use_case = null ) {
	...
	$is_excerpt = doing_filter( 'get_the_excerpt' );
	...
	if ( is_editor_page() ) {
		...
		$tve_saved_content = tve_get_post_meta( $post_id, "tve_save_post", true );
		if ( ! function_exists( 'quicklatex_parser' ) ) {
			/**
			 * SUPP-3172 this plugin needs backslashes
			 */
		...
		if ( in_the_loop() || $is_landing_page ) {
			add_action( 'wp_footer', 'tve_output_wysiwyg_editor', 9 );
		}
		...
		if ( $is_excerpt ) {
			$tve_saved_content = tve_prepare_excerpt( $tve_saved_content );
		}
	}
...
/**
 *
 * Prepares the content for being displayed as excerpt. Should strip out text that should not be visible in the excerpt displays
 *
 * @param string $content
 *
 * @return string
 */
function tve_prepare_excerpt( $content ) {
	/**
	 * Texts from social sharing element
	 */
	$content = preg_replace( '#<span class="(tve_s_text|tve_s_count)">(.*?)</span>#is', '', $content );

	/**
	 * Texts from the Lead Generation element
	 */
	$content = preg_replace( '#<button type="submit">(.+?)</button>#is', '', $content );
			...
			if ( apply_filters( 'tcb_user_can_edit', true, $post_id ) ) {
				...
				$tve_path_params = array(
					...
					'translations'                  => array(
						...
						"LeadGenerationPassword"               => __( "Please enter a password", 'thrive-cb' ),
						"LeadGenerationPasswordMismatch"       => __( "The passwords do not match", 'thrive-cb' ),
						...
						'PinRequiredImage'                     => __( 'Please select or upload an image, this field is required for pinterest pin' ),
...
function tve_render_widget_menu( $attributes ) {
	...
	if ( ! empty( $link_custom_color ) || ! empty( $top_link_custom_color ) ) {
		...
		$GLOBALS['tve_menu_top_link_custom_color'] = empty( $top_link_custom_color ) ? $link_custom_color : $top_link_custom_color;
	...
	$menu_html = '<div class="thrive-shortcode-html tve_clearfix"><a' . $trigger_color . ' class="tve-m-trigger t_' . $attributes['dir'] . ' ' .
			...
			'items_wrap'     => '<nav><ul' . $ul_custom_color . ' id="%1$s" class="%2$s"' . ( ! empty( $attributes['font_size'] ) ? ' style="font-size:' . $attributes['font_size'] . '"' : '' ) . '>%3$s</ul></nav>',
...
function tve_find_quick_link_contents() {
	...
	$display_tqb_tab = is_plugin_active( 'thrive-quiz-builder/thrive-quiz-builder.php' ) && ! empty( $_REQUEST['tqb_key'] );
	$display_tqb_tab = $display_tqb_tab && ! empty( $_REQUEST['tqb_key'] );
	$display_tqb_tab = $display_tqb_tab && ! empty( $_REQUEST['post_type'] ) && $_REQUEST['post_type'] !== Thrive_Quiz_Builder::QUIZ_STRUCTURE_ITEM_RESULTS;

	include dirname( __FILE__ ) . '/views/quick-links-table.php';
	exit;
}

function tve_autocomplete_posts() {
	$s = wp_unslash( $_REQUEST['q'] );
	$s = trim( $s );
	...
	$exceptionList = apply_filters( 'tve_post_types_blacklist', array( 'attachment', 'focus_area', 'thrive_optin', 'tcb_lightbox', 'wysijap', 'tve_lead_2s_lightbox' ) );
	...
	foreach ( $posts_array as $id => $item ) {
		if ( ! empty( $s ) ) {
			$title = preg_replace( "#($s)#i", "<b>$0</b>", $item->post_title );
		}
...
function tve_api_form_submit() {
	...
	if ( $data['_submit_option'] == 'page' ) {
		$id                 = intval( $data['_back_url'] );
		$result['back_url'] = get_permalink( $id );
	}
	...
	/**
	 * action filter -  allows hooking after the form submission event
	 *
	 * @param array $post   the full _POST data
	 * @param array $result the result
	 *
	 */
	do_action( 'tcb_api_after_form_submit', $post, $result );
